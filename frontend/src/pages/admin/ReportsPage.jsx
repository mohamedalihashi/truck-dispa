...
export function ReportsPage() {
  ...
  }, [type, trucks.data, trucks.isLoading, users.data, users.isLoading, trips.data, trips.isLoading, payments.data, payments.isLoading]);

  const report = useMemo(() => {
    const needle = reportSearch.trim().toLowerCase();
    const rows = baseReport.rows.filter((row) => {
      if (type === "users" && reportUserId && row.id !== reportUserId) return false;
    ...
    return { ...baseReport, rows };
  }, [baseReport, reportSearch, reportUserId, dateFrom, dateTo, type]);
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2 print:hidden">
            <Button variant="outline" onClick={() => window.print()}><Printer size={17} /> Print</Button>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4 print:hidden">
        {Object.entries(REPORTS).map(([key, item]) => (
          <button
            ...
            type="button"
            onClick={() => setType(key)}
            className={`rounded-xl border p-4 text-left transition ${type === key ? "border-secondary-container bg-secondary-container text-on-secondary-container shadow-md" : "border-outline-variant bg-surface-container-lowest hover:border-secondary-container"}`}
          >
            <span className="font-semibold">{item.label}</span>
            <span className={`mt-1 block text-xs ${type === key ? "text-on-secondary-container/80" : "text-on-surface-variant"}`}>{item.description}</span>
          </button>
        ))}
      </div>
      ...
      <section className="mb-6 rounded-xl border border-outline-variant bg-surface-container-lowest p-5 shadow-sm print:hidden">
        <div className="mb-4">
          <h2 className="font-semibold text-primary-container">Search and filter report</h2>
          <p className="mt-1 text-xs text-on-surface-variant">
            Search by Truck ID, truck number, user, customer, driver, dispatcher, phone, or email.
          </p>
        </div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <label>
              ...
              <select className="stitch-input" value={reportUserId} onChange={(event) => setReportUserId(event.target.value)}>
                ...
                {(users.data?.data || []).map((user) => (
                  <option key={user.id} value={user.id}>{user.name} · {user.role} · {user.phone || user.email}</option>
                ))}
              </select>
            </label>
          )}
          <label>
            ...
            <input className="stitch-input" type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} max={dateTo || undefined} />
          </label>
          <label>
            <span className="mb-1 block text-xs font-semibold text-on-surface-variant">Date to</span>
            <input className="stitch-input" type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} min={dateFrom || undefined} />
          </label>
        </div>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-on-surface-variant"><span className="font-bold text-primary-container">{report.rows.length}</span> matching records</p>
      ...
      <section className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {report.metrics.map(([label, value]) => (
          <div key={label} className="rounded-xl border border-outline-variant bg-surface-container-lowest p-5 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-bold text-primary-container">{value}</p>
          </div>
        ))}
      </section>

      {type === "users" && <div className="mb-6"><UserActivityReport /></div>}

      <section className="mb-6 rounded-xl border border-outline-variant bg-surface-container-lowest p-6 shadow-sm">
        <h2 className="mb-5 text-xl font-semibold text-primary-container">{report.chartTitle}</h2>
        <div className="h-72 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={report.chartData} margin={{ top: 5, right: 12, left: 0, bottom: 5 }}>
              ...
              <Bar dataKey="value" name={type === "payments" ? "Amount" : "Records"} fill="#a04100" radius={[8, 8, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant px-6 py-5">
          <div>
            <h2 className="text-xl font-semibold text-primary-container">{REPORTS[type].label}</h2>
            <p className="mt-1 text-sm text-on-surface-variant">Generated {new Date().toLocaleString()} · {report.rows.length} records</p>
          </div>
        </div>
        {report.loading ? <p className="px-6 py-10 text-center text-on-surface-variant">Preparing report…</p> : <DataTable columns={report.columns} rows={report.rows} empty="No data available for this report." />}
      </section>
